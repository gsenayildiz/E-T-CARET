...
//! RENDER ürünleri ekrana aktaracak fonksiyon 
...
function addToCart(event){
    //sepete eklenecek olan ürünün id sine erişme
    const productID = parseInt(event.target.dataset.id);
    //find metodu ile dizi içindeki ürünün id si bulundu
   const product = products.find((product) => product.id === productID);
    
 if(product){
    //sepete eklenilen üründen, öncesinde sepette var mı yok mu 
    const exixtingItem = cart.find((item) => item.id === productID);
    //varsa ürün miktari bir artsın
  if(exixtingItem){
    exixtingItem.quantity++;
   //yoksa yeni ürün şeklinde eklenir
  }else{
    const cartItem = {
        id: product.id,
        title: product.title,
...
//! LOCALSTORAGE DAN VE CART DİZİSİNDEN ÜRÜN SİLME İŞLEMİ
function removeFromCart(event){
 const productID = parseInt(event.target.dataset.id);
console.log(cart);
//cart dizisinden ürünü id sine göre sildik
...
// sayfa güncellendi
renderCartItems();
//sepetteki güncel fiyatı günceller
calculateCartTotal();
...
//!sayfa cart.html sayfasındaysa renderCartItems fonksiyonu çalışması gerekiyor
if(window.location.pathname.includes("cart.html")){
    renderCartItems();
    calculateCartTotal();
}else{
    //sayfa index.html sayfasındaysa renderProducts fonksiyonu çalışır
    renderProducts();
}
